...
function canBeContinuedListItem(lineText: string) {
	...
	return (
		reBulletListMarker.test(leftTrimmed) ||
		reOrderedListMarker.test(leftTrimmed)
	);
}
...
export class ToastMark implements ToastMarkParser {
	...
	private updateLineTexts(startPos: Pos, endPos: Pos, newText: string) {
		...
		newLines[newLineLen - 1] =
			newLines[newLineLen - 1] + (endLineText?.slice(endCol - 1) || "");
	...
	private updateRootNodeState() {
		...
		if (this.root.lastChild) {
			this.root.lastLineBlank = (
				this.root.lastChild as BlockNode
			).lastLineBlank;
		}
		...
		this.root.sourcepos![1] = [idx + 1, lineTexts[idx]?.length || 0];
	}
	...
	private getNodeRange(startPos: Pos, endPos: Pos) {
		...
		if (
			endNode &&
			endNode.next &&
			endPos[0] + 1 === endNode.next.sourcepos![0][0]
		) {
	...
	private trigger(eventName: EventName, param: any) {
		for (const handler of this.eventHandlerMap[eventName]) {
			//@ts-ignore
			handler(param);
		}
	}
	...
	private parseRange(
		...
		if (
			...
			((isList(startNode.prev) &&
				...
				(isTable(startNode.prev) &&
					canBeContinuedTableBody(this.lineTexts[startLine - 1]!)))
		) {
		...
		// extends ending range if the followig node can be a fenced code block or a continued list item
		let nextNode = endNode ? endNode.next : this.root.firstChild;
		const { lastChild } = root;
		const isOpenedLastChildCodeBlock =
			lastChild && isCodeBlock(lastChild) && lastChild.open;
		...
		while (
			((isOpenedLastChildCodeBlock || isOpenedLastChildCustomBlock) &&
				nextNode) ||
			(isLastChildList &&
				nextNode &&
			...
			this.parser.partialParseExtends(
				this.lineTexts.slice(endLine, newEndLine),
			);
	...
	private parse(startPos: Pos, endPos: Pos, lineDiff = 0): ParseResult {
		...
		const endLine = this.extendEndLine(
			(endNode ? Math.max(endNode.sourcepos![1][0], endPos[0]) : endPos[0]) +
				lineDiff,
		);

		const parseResult = this.parseRange(
			...this.getRangeWithRefDef(
				startLine,
				endLine,
				startNode,
				endNode,
	...
	private removeUnlinkedCandidate() {
		if (!isEmptyObj(this.refDefCandidateMap)) {
			[this.refLinkCandidateMap, this.refDefCandidateMap].forEach(
				(candidateMap) => {
