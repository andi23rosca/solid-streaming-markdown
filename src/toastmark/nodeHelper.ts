...
import { type Node, getNodeById, removeNodeById } from "./commonmark/node";
...
export function findChildNodeAtLine(parent: Node, line: number) {
	...
	while (node) {
		const comp = compareRangeAndLine(
			node.sourcepos ?? [
				[1, 1],
				[1, 1],
			],
...
export function toString(node: Node | null) {
	...
	}, lastChild: ${node.lastChild?.type}, prev: ${
		node.prev?.type
	}, next: ${node.next?.type}`;
}
