import type { InlineNodeType, Sourcepos, CustomBlockMdNode } from "@t/node";
import type {
	RefMap,
	RefLinkCandidateMap,
	RefDefCandidateMap,
...
export class InlineParser {
	...
	parseBackticks(block: BlockNode) {
		...
		while ((matched = this.match(reTicks)) !== null) {
			if (matched === ticks) {
				let contents = this.subject.slice(
					afterOpenTicks,
	...
	parseBackslash(block: BlockNode) {
		...
		if (this.peek() === C_NEWLINE) {
			...
			block.appendChild(
				text(subj.charAt(this.pos), this.sourcepos(startpos, this.pos)),
			);
			this.pos += 1;
		} else {
	...
	scanDelims(cc: number) {
		...
		if (
			numdelims === 0 ||
			(numdelims < 2 && (cc === C_TILDE || cc === C_DOLLAR))
		) {
			this.pos = startpos;
			return null;
		}
		...
		while (closer !== null) {
			...
			if (!closer.canClose) {
				...
				while (
					...
					opener !== stackBottom &&
					opener !==
						openersBottom[closercc][closerEmph ? closer.origdelims % 3 : 0]
				) {
					oddMatch =
						closerEmph &&
	...
	parseEntity(block: BlockNode) {
		...
		if ((m = this.match(reEntityHere))) {
			block.appendChild(
				text(decodeHTML(m), this.sourcepos(startpos, this.pos)),
			);
			return true;
		}
		return false;
	...
	parseString(block: BlockNode) {
		...
		if ((m = this.match(reMain))) {
			if (this.options.smart) {
				const lit = m.replace(reEllipses, "\u2026").replace(reDash, (chars) => {
					let enCount = 0;
					let emCount = 0;
					if (chars.length % 3 === 0) {
	...
	parseNewline(block: BlockNode) {
		...
		if (
			...
			block.appendChild(
				createNode("softbreak", this.sourcepos(this.pos, this.pos)),
			);
		}
		this.nextLine();
	...
	parseInline(block: BlockNode) {
		...
		if (!res) {
			this.pos += 1;
			block.appendChild(
				text(fromCodePoint(c), this.sourcepos(this.pos, this.pos + 1)),
			);
		}
